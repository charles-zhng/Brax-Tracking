...
def get_quat(theta=0, rot_axis=[0., 0, 1]):
    ...
    """
    axis = rot_axis / jp.linalg.norm(rot_axis)
    c = jp.cos(theta / 2)
    s = jp.sin(theta / 2)
    return jp.array((c, axis[0] * s, axis[1] * s, axis[2] * s))


def random_quat():
    """Returns normalized random quaternion."""
    theta = 2 * jp.pi * jp.random.rand()
    axis = 2 * jp.random.rand(3) - 1
...
def mult_quat(quat1: jp.ndarray, quat2: jp.ndarray) -> jp.ndarray:
...
def conj_quat(quat: jp.ndarray) -> jp.ndarray:
...
def reciprocal_quat(quat: jp.ndarray) -> jp.ndarray:
    ...
    """
    return conj_quat(quat) / jp.linalg.norm(quat, axis=-1, keepdims=True)**2
...
def vec_world_to_local(world_vec, root_quat, hover_up_dir_quat=None):
    ...
    if hover_up_dir_quat is not None:
        # For flexible broadcasting, instead of jp.tile
        hover_up_dir_quat = jp.zeros_like(root_quat) + hover_up_dir_quat
        root_quat = mult_quat(conj_quat(hover_up_dir_quat), root_quat)

    return rotate_vec_with_quat(world_vec, root_quat)


def log_quat(quat: jp.ndarray) -> jp.ndarray:
    ...
    Returns:
        Array of shape (B, 4).
    """
    norm_quat = jp.linalg.norm(quat, axis=-1, keepdims=True)
    norm_v = jp.linalg.norm(quat[..., 1:], axis=-1, keepdims=True)
    log_quat = jp.empty_like(quat)
    log_quat[..., 0:1] = jp.log(norm_quat)
    log_quat[..., 1:] = quat[..., 1:] / norm_v * jp.arccos(
        quat[..., 0:1] / norm_quat)
    return log_quat


def quat_z2vec(vec: jp.ndarray) -> jp.ndarray:
...
def axis_angle_to_quat(axis: jp.ndarray, angle: jp.ndarray) -> jp.ndarray:
...
def quat_dist_short_arc(quat1: jp.ndarray, quat2: jp.ndarray) -> jp.ndarray:
    ...
    """
    quat1 = quat1 / jp.linalg.norm(quat1, axis=-1, keepdims=True)
    quat2 = quat2 / jp.linalg.norm(quat2, axis=-1, keepdims=True)
    x = 2 * jp.sum(quat1 * quat2, axis=-1)**2 - 1
    x = jp.minimum(1., x)
    return jp.arccos(x)


def joint_orientation_quat(xaxis: jp.ndarray, qpos: float) -> jp.ndarray:
...
def quat_to_angvel(quat, dt=1.):
    ...
    Returns:
        Angular velocity vector, (B, 3), in the same (global) reference frame
            as the input quat.
    """
    sin_a_2 = jp.linalg.norm(quat[..., 1:], axis=-1, keepdims=True)
    axis = quat[..., 1:] / sin_a_2  # Normalize.
    speed = 2 * jp.arctan2(sin_a_2, quat[..., 0:1])
    # When axis-angle is larger than pi, rotation is in opposite direction.
    if speed.shape:
        speed[speed > jp.pi] -= 2 * jp.pi  # speed is vector.
    elif speed > jp.pi:
        speed -= 2 * jp.pi  # speed is scalar.
